...
/**
 * File:        UpdateInstaller.java
 * Description: Downloads an update APK via DownloadManager, reporting live
 *              progress (percent/bytes/speed) to a listener, and hands the
 *              finished file to the system installer via
 *              DownloadManager.getUriForDownloadedFile() - a content://
 *              URI backed by the DownloadProvider itself, so no
 *              FileProvider/manifest <provider> is needed to share it.
 *              Same pattern as the other FuZz apps (FuZz LED, LEDCAR) - see
 *              MainActivity.startUpdateDownload() for how this replaces the
 *              old "open a browser to the raw APK URL" flow.
 * Author:      FuzzBC
 * Date:        2026-09-02
 */
public class UpdateInstaller {
    ...
    public void download(String apkUrl, String versionTag, ProgressListener listener) {
        ...
        String fileName = "FuZzVolumeHU_" + versionTag + ".apk";
        DownloadManager.Request req = new DownloadManager.Request(Uri.parse(apkUrl))
                .setTitle("FuZz Volume HU " + versionTag)
